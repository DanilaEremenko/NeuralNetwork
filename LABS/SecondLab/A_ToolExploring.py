...
if __name__ == '__main__':
    ...
    print("\n--------------Zeros--------------\n")
    model = Sequential()
    model.add(Dense(ne_num, input_dim=1, kernel_initializer=wi.Zeros(), bias_initializer=wi.Zeros()))
    print(model.get_weights())

    print("\n--------------Ones--------------\n")
    model = Sequential()
    model.add(Dense(ne_num, input_dim=1, kernel_initializer=wi.Ones(), bias_initializer=wi.Ones()))
    print(model.get_weights())

    print("\n--------------Constant--------------\n")
    model = Sequential()
    model.add(
        Dense(ne_num, input_dim=1, kernel_initializer=wi.Constant(value=2.0), bias_initializer=wi.Constant(value=2.0)))
    print(model.get_weights())

    print("\n--------------RandomNormal--------------\n")
    model = Sequential()
    model.add(Dense(ne_num, input_dim=1, kernel_initializer=wi.RandomNormal(mean=0.0, stddev=0.05, seed=seed),
                    bias_initializer=wi.RandomNormal(mean=0.0, stddev=0.05, seed=seed)))
    print(model.get_weights())

    print("\n--------------RandomUniform--------------\n")
    model = Sequential()
    model.add(Dense(ne_num, input_dim=1, kernel_initializer=wi.RandomUniform(minval=-0.05, maxval=0.05, seed=seed),
                    bias_initializer=wi.RandomUniform(minval=-0.05, maxval=0.05, seed=seed)))
    print(model.get_weights())

    print("\n--------------TruncatedNormal--------------\n")
    model = Sequential()
    model.add(Dense(ne_num, input_dim=1, kernel_initializer=wi.TruncatedNormal(mean=0.0, stddev=0.05, seed=seed),
